...
class SimulationManager {
    ...
    // Before metrics
    const beforeMetrics = document.querySelectorAll('.comparison-item.before .metric-value');
    if (beforeMetrics.length >= 3) {
        beforeMetrics[0].textContent = results.before.emissions;
        beforeMetrics[1].textContent = results.before.energy + 'K';
        beforeMetrics[2].textContent = '$' + (results.before.cost / 1000).toFixed(1) + 'M';
    }

    // After metrics
    const afterMetrics = document.querySelectorAll('.comparison-item.after .metric-value');
    if (afterMetrics.length >= 3) {
        afterMetrics[0].textContent = results.after.emissions;
        afterMetrics[1].textContent = results.after.energy + 'K';
        afterMetrics[2].textContent = '$' + (results.after.cost / 1000).toFixed(1) + 'M';
    }
    ...
    const savings = document.querySelectorAll('.saving-value');
    if (savings.length >= 3) {
        savings[0].textContent = `-${emissionReduction}%`;
        savings[1].textContent = `-${energyReduction}%`;
        savings[2].textContent = `$${(results.savings.money / 1000000).toFixed(1)}M`;
    }
